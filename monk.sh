...
if [[ ${OPTION} == "1" ]] ; then
  ...
  read ALIAS
  wget https://raw.githubusercontent.com/CryptoCatOkiOKi/MonkeyProjectScripts/master/monk_resync.sh -O monk_resync.sh > /dev/null 2>&1
  chmod 777 monk_resync.sh
  dos2unix monk_resync.sh > /dev/null 2>&1
  /bin/bash ./monk_resync.sh $ALIAS
elif [[ ${OPTION} == "9" ]] ; then  
  wget https://raw.githubusercontent.com/CryptoCatOkiOKi/MonkeyProjectScripts/master/monkey_logo.sh -O monkey_logo.sh > /dev/null 2>&1
